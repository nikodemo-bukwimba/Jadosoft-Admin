...
<#
.SYNOPSIS
    Inserts content above a marker line in a file.
    Matches any line that STARTS WITH the marker text (ignores trailing dashes/chars).
#>
function _Insert-AboveMarker {
    ...
    foreach ($line in $lines) {
        if (-not $inserted -and $line.TrimStart().StartsWith($Marker.TrimStart())) {
            foreach ($iLine in ($Insert -split "`n")) {
                $result.Add($iLine)
            }
            $inserted = $true
        }
...
function Update-InjectionContainer {
    ...
    $imports.Add("import 'package:fca/features/${fname}/domain/workflow/${fname}_workflow_executor.dart';")
    ...
    $regs.Add("  ));")
    $regs.Add("  sl.registerLazySingleton(() => ${fclass}WorkflowExecutor());")
...
function Update-ShellNavItems {
    ...
    $tab = @"
      // $flabel tab (Level 3, generated $(Get-Date -Format 'yyyy-MM-dd'))
      ShellTabConfig(
        label:      '$flabel',
        icon:       $icon,
