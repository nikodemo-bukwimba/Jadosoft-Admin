...
function Get-ValidationLines {
    param([Parameter(Mandatory)][string]$FieldName, [Parameter(Mandatory)][string]$ConfigType,
        [psobject]$Validation, [string]$Accessor = "p.$FieldName", [bool]$IsNullable = $false)
    ...
    if ($req -and $req.Value.value -eq $true) {
        ...
        elseif ($IsNullable) {
            # Only emit null check when the Dart type is actually nullable
            $lines.Add("    if ($Accessor == null) {")
            $lines.Add("      return const Left(ValidationFailure('$msg'));")
            $lines.Add("    }")
        }
        # Non-nullable non-String: Dart's type system already enforces presence, skip guard
    }
