...
function _Get-DetailAccessor {
  ...
  if ($nullable) {
    switch ($t) {
      'DateTime' { return "item." + $n + " != null ? item." + $n + "!.toIso8601String().split('T').first : ''" }
      'bool' { return "item." + $n + "?.toString() ?? ''" }
      'int' { return "item." + $n + "?.toString() ?? ''" }
      'double' { return "item." + $n + "?.toStringAsFixed(2) ?? ''" }
      default { return "item." + $n + " ?? ''" }
    }
  }
  else {
    switch ($t) {
      'DateTime' { return "item." + $n + ".toIso8601String().split('T').first" }
      'bool' { return "item." + $n + ".toString()" }
      'int' { return "item." + $n + ".toString()" }
      'double' { return "item." + $n + ".toStringAsFixed(2)" }
      default { return "item." + $n }
...
"@)
  ...
  # Build updatedItem reload line via concat (Dart ! operator)
  $reloadLine = '              context.read<' + $fclass + 'Bloc>()'
  $reloadAdd = '                  .add(' + $fclass + 'LoadOneRequested(state.updatedItem!.id));'
