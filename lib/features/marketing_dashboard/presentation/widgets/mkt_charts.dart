    ...
    return GridView.count(
      ...
      children: [
        ...
        KpiCard(
          ...
          delta: '${projection.visitsByOfficer.length} tracked',
          deltaPositive: true,
        ...
        KpiCard(
          ...
          value: '${(projection.planComplianceRate * 100).round()}%',
          delta: projection.planComplianceRate >= 0.7
              ? 'On Track'
              : 'Below target',
          deltaPositive: projection.planComplianceRate >= 0.7,
          icon: Icons.fact_check_outlined,
          color: Colors.orange.shade600,
        ),
        KpiCard(
          title: 'Reports Filed',
          value: projection.dailyReportSubmissionRate.toString(),
          delta: 'this period',
          deltaPositive: true,
  ...
  Widget build(BuildContext context) {
    ...
    for (final v in projection.recentVisits) {
      final key = [
        ...
        'Sat',
        'Sun',
      ][v.visitDate.weekday - 1];
      if (visitsByDay.containsKey(key))
        visitsByDay[key] = (visitsByDay[key] ?? 0) + 1;
    }

    final hasData = visitsByDay.values.any((v) => v > 0);
    final maxVal = hasData
        ? visitsByDay.values.reduce((a, b) => a > b ? a : b)
        : 0;
...
class MktComplianceRing extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // Real counts from projection
    final approvedCount = projection.approvedPlanCount;
    final pendingCount = projection.pendingPlanCount;
    final rejectedCount = projection.rejectedPlanCount;
    final totalPlans = approvedCount + pendingCount + rejectedCount;

    String _pct(int count) =>
        totalPlans > 0 ? '${(count / totalPlans * 100).round()}%' : '0%';
                ...
                child: Column(
                  ...
                  children: [
                    _LegendRow(
                      color: Colors.green,
                      label: 'Approved',
                      value: _pct(approvedCount),
                    ),
                    const SizedBox(height: 8),
                    _LegendRow(
                      color: Colors.orange,
                      label: 'Pending',
                      value: _pct(pendingCount),
                    ),
                    const SizedBox(height: 8),
                    _LegendRow(
                      color: Colors.red,
                      label: 'Rejected',
                      value: _pct(rejectedCount),
                    ),
                    ...
                    Container(
                      ...
                      child: Row(
                        children: [
                          Icon(Icons.storefront, size: 16, color: cs.primary),
                          ...
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                Text(
                                  '${projection.customersVisitedThisMonth}/${projection.totalCustomers}',
                                  style: TextStyle(
                                    fontSize: 14,
                                    fontWeight: FontWeight.w700,
                                    color: cs.primary,
