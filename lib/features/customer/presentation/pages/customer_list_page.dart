...
class _CustomerListPageState extends State<CustomerListPage> {
    ...
    if (w >= 1024) {
      return GridView.builder(
        ...
        gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
          crossAxisCount: w >= 1200 ? 3 : 2, childAspectRatio: 2.0, crossAxisSpacing: 12, mainAxisSpacing: 0),
        itemCount: items.length,
        itemBuilder: (_, i) => CustomerCardTile(
          item: items[i], onTap: () => _navigateToDetail(items[i].id),
          onDelete: () => _deleteCustomer(items[i].id, items[i].businessName)),
      );
    }
    ...
    return SingleChildScrollView(
      ...
      child: Column(children: [
        Container(
          ...
          child: Row(children: [
            const SizedBox(width: 44), const SizedBox(width: 12),
            Expanded(flex: 3, child: Text('Business', style: _hdr)),
            Expanded(flex: 2, child: Text('Owner', style: _hdr)),
            Expanded(flex: 2, child: Text('Phone', style: _hdr)),
            const SizedBox(width: 40),
          ]),
        ),
        ...items.asMap().entries.map((e) => CustomerTableRow(
          item: e.value, isLast: e.key == items.length - 1,
          onTap: () => _navigateToDetail(e.value.id),
          onDelete: () => _deleteCustomer(e.value.id, e.value.businessName))),
      ]),
    );
  }

  TextStyle get _hdr => TextStyle(fontSize: 11, fontWeight: FontWeight.w700, letterSpacing: 0.5,
  ...
  Widget _buildEmpty(BuildContext context, ColorScheme scheme) => Center(
    child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      ...
      Text('Tap + to add your first customer.', style: Theme.of(context).textTheme.bodyMedium?.copyWith(color: scheme.onSurfaceVariant)),
    ]),
  );

  Widget _buildError(BuildContext context, ColorScheme scheme, String msg) => Center(
    child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
