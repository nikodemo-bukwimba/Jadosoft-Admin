...
class OfficerDetailPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Officer Detail'), actions: [
        BlocBuilder<OfficerBloc, OfficerState>(builder: (context, state) {
      ...
      body: BlocConsumer<OfficerBloc, OfficerState>(
        ...
        builder: (context, state) {
          ...
          if (state is OfficerFailure) return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          ...
          if (state is OfficerDetailLoaded) return _buildContent(context, state.item);
          return const SizedBox.shrink();
        },
      ),
    );
      ...
      child: ConstrainedBox(constraints: const BoxConstraints(maxWidth: 720), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Card(child: Padding(padding: const EdgeInsets.all(24), child: Row(children: [
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(item.displayName, style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.w800)),
            const SizedBox(height: 4),
            Text(item.orgRoleName ?? '', style: Theme.of(context).textTheme.titleSmall?.copyWith(color: scheme.primary, fontWeight: FontWeight.w600)),
            if (item.branchName != null) ...[const SizedBox(height: 2),
              Text(item.branchName!, style: Theme.of(context).textTheme.bodySmall?.copyWith(color: scheme.onSurfaceVariant))],
        ...
        Card(child: Padding(padding: const EdgeInsets.all(20), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text('Contact Information', style: Theme.of(context).textTheme.titleSmall?.copyWith(color: scheme.primary, fontWeight: FontWeight.w700)),
          const Divider(height: 24),
          _contactRow(context, Icons.email_outlined, 'Email', item.email),
          _contactRow(context, Icons.phone_outlined, 'Phone', item.phone ?? 'N/A'),
          _contactRow(context, Icons.badge_outlined, 'Role', item.orgRoleName ?? 'N/A'),
          _contactRow(context, Icons.business_outlined, 'Branch', item.branchName ?? 'N/A'),
          _contactRow(context, Icons.calendar_today_outlined, 'Joined', item.createdAt?.toIso8601String().split('T').first ?? 'N/A'),
          _contactRow(context, Icons.fingerprint, 'User ID', item.userId),
          _contactRow(context, Icons.perm_identity, 'Actor ID', item.actorId),
        ]))),
  ...
  Widget _buildActions(BuildContext context, OfficerEntity item, OfficerStatus statusEnum) {
    ...
    if (statusEnum == OfficerStatus.suspended) {
      actions.add(_actionButton(context, icon: Icons.check_circle_outline, label: 'Activate', color: Colors.green,
        onPressed: () => context.read<OfficerBloc>().add(OfficerActivateRequested(item.userId))));
    }
    if (statusEnum == OfficerStatus.active) {
      actions.add(_actionButton(context, icon: Icons.pause_circle_outline, label: 'Suspend', color: Colors.orange,
    ...
    if (statusEnum == OfficerStatus.active || statusEnum == OfficerStatus.suspended) {
      actions.add(_actionButton(context, icon: Icons.cancel_outlined, label: 'Deactivate', color: Colors.grey,
    ...
    return Card(child: Padding(padding: const EdgeInsets.all(20), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text('Actions', style: Theme.of(context).textTheme.titleSmall?.copyWith(color: Theme.of(context).colorScheme.primary, fontWeight: FontWeight.w700)),
  ...
  Widget _actionButton(BuildContext context, {required IconData icon, required String label, required Color color, required VoidCallback onPressed}) {
    return FilledButton.tonalIcon(onPressed: onPressed, icon: Icon(icon, size: 18, color: color), label: Text(label),
  ...
  Widget _contactRow(BuildContext context, IconData icon, String label, String value) {
    ...
    return Padding(padding: const EdgeInsets.symmetric(vertical: 8), child: Row(children: [
      ...
      SizedBox(width: 80, child: Text(label, style: Theme.of(context).textTheme.bodySmall?.copyWith(color: scheme.onSurfaceVariant))),
  ...
  Future<void> _confirmDelete(BuildContext context, String userId, String name) async {
    ...
    if (confirmed == true && context.mounted) context.read<OfficerBloc>().add(OfficerDeleteRequested(userId));
  }
}
