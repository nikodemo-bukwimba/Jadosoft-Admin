...
class _WeeklyPlanListPageState extends State<WeeklyPlanListPage> {
  ...
  @override
  void initState() { super.initState(); context.read<WeeklyPlanBloc>().add(WeeklyPlanLoadAllRequested()); }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Weekly Plans'), actions: [_toggle(scheme), const SizedBox(width: 8)]),
      body: BlocConsumer<WeeklyPlanBloc, WeeklyPlanState>(
        listener: (c, s) {
          if (s is WeeklyPlanOperationSuccess) { ScaffoldMessenger.of(c).showSnackBar(SnackBar(content: Text(s.message))); c.read<WeeklyPlanBloc>().add(WeeklyPlanLoadAllRequested()); }
          if (s is WeeklyPlanFailure) ScaffoldMessenger.of(c).showSnackBar(SnackBar(content: Text(s.message), backgroundColor: scheme.error));
        },
        builder: (c, s) {
          if (s is WeeklyPlanLoading || s is WeeklyPlanInitial) return const Center(child: CircularProgressIndicator());
          if (s is WeeklyPlanEmpty) return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
            Icon(Icons.event_busy, size: 64, color: scheme.outlineVariant), const SizedBox(height: 16),
            Text('No weekly plans submitted yet.', style: Theme.of(c).textTheme.titleMedium?.copyWith(color: scheme.onSurfaceVariant)),
          ]));
          if (s is WeeklyPlanFailure) return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
            Icon(Icons.error_outline, size: 48, color: scheme.error), const SizedBox(height: 16), Text(s.message),
            const SizedBox(height: 16), FilledButton.icon(onPressed: () => c.read<WeeklyPlanBloc>().add(WeeklyPlanLoadAllRequested()), icon: const Icon(Icons.refresh), label: const Text('Retry')),
          ]));
          if (s is WeeklyPlanListLoaded) return RefreshIndicator(
            onRefresh: () async => c.read<WeeklyPlanBloc>().add(WeeklyPlanLoadAllRequested()), child: _buildView(s.items));
  ...
  Widget _cards(List<WeeklyPlanEntity> items) {
    ...
    if (w >= 1024) return GridView.builder(padding: const EdgeInsets.all(16),
      ...
      itemCount: items.length, itemBuilder: (_, i) => WeeklyPlanCardTile(item: items[i], onTap: () => _nav(items[i].id)));
    return ListView.builder(padding: const EdgeInsets.all(16), itemCount: items.length,
      itemBuilder: (_, i) => WeeklyPlanCardTile(item: items[i], onTap: () => _nav(items[i].id)));
  }

  Widget _list(List<WeeklyPlanEntity> items) => ListView.builder(padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
    itemCount: items.length, itemBuilder: (_, i) => WeeklyPlanListRow(item: items[i], onTap: () => _nav(items[i].id)));
  ...
  Widget _details(List<WeeklyPlanEntity> items) {
    ...
    return SingleChildScrollView(padding: const EdgeInsets.all(16), child: Column(children: [
      Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 10),
        ...
        child: Row(children: [const SizedBox(width: 40), const SizedBox(width: 12), Expanded(flex: 3, child: Text('Week', style: hdr)),
          Expanded(flex: 1, child: Text('Targets', style: hdr)), Expanded(flex: 1, child: Text('Status', style: hdr))])),
      ...items.asMap().entries.map((e) => WeeklyPlanTableRow(item: e.value, isLast: e.key == items.length - 1, onTap: () => _nav(e.value.id))),
    ]));
  }
}