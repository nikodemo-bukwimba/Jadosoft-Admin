import 'package:flutter/material.dart';
import 'package:flutter_bloc/flutter_bloc.dart';
import '../bloc/promotion_bloc.dart';
import '../bloc/promotion_event.dart';
import '../bloc/promotion_state.dart';
import '../widgets/promotion_card.dart';

class PromotionListPage extends StatefulWidget {
  const PromotionListPage({super.key});

  @override
  State<PromotionListPage> createState() => _PromotionListPageState();
}

class _PromotionListPageState extends State<PromotionListPage> {
  @override
  void initState() {
    super.initState();
    context.read<PromotionBloc>().add(PromotionLoadAllRequested());
  }

  @override
  Widget build(BuildContext context) {
    final scheme = Theme.of(context).colorScheme;

    return Scaffold(
      appBar: AppBar(title: const Text('Promotionss')),
      floatingActionButton: FloatingActionButton(
        onPressed: () => Navigator.of(context)
            .pushNamed('/promotions/create')
            .then((_) => context.read<PromotionBloc>().add(PromotionLoadAllRequested())),
        child: const Icon(Icons.add),
      ),
      body: BlocConsumer<PromotionBloc, PromotionState>(
        listener: (context, state) {
          if (state is PromotionOperationSuccess) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(content: Text(state.message)),
            );
            context.read<PromotionBloc>().add(PromotionLoadAllRequested());
          }
          if (state is PromotionFailure) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(content: Text(state.message), backgroundColor: scheme.error),
            );
          }
        },
        builder: (context, state) {
          if (state is PromotionLoading || state is PromotionInitial) {
            return const Center(child: CircularProgressIndicator());
          }
          if (state is PromotionEmpty) {
            return Center(
              child: Column(
                mainAxisAlignment: MainAxisAlignment.center,
                children: [
                  Icon(Icons.inbox_outlined, size: 64, color: scheme.outlineVariant),
                  const SizedBox(height: 16),
                  Text('No promotions yet. Create your first promotion.', style: Theme.of(context).textTheme.bodyLarge),
                ],
              ),
            );
          }
          if (state is PromotionFailure) {
            return Center(
              child: Column(
                mainAxisAlignment: MainAxisAlignment.center,
                children: [
                  Text(state.message),
                  const SizedBox(height: 16),
                  FilledButton(
                    onPressed: () => context.read<PromotionBloc>().add(PromotionLoadAllRequested()),
                    child: const Text('Retry'),
                  ),
                ],
              ),
            );
          }
          if (state is PromotionListLoaded) {
            return RefreshIndicator(
              onRefresh: () async =>
                  context.read<PromotionBloc>().add(PromotionLoadAllRequested()),
              child: ListView.builder(
                padding: const EdgeInsets.all(16),
                itemCount: state.items.length,
                itemBuilder: (_, i) => PromotionCard(
                  item: state.items[i],
                  onTap: () => Navigator.of(context)
                      .pushNamed('/promotions/detail', arguments: {'id': state.items[i].id})
                      .then((_) => context.read<PromotionBloc>().add(PromotionLoadAllRequested())),
                  onDelete: () => context.read<PromotionBloc>()
                      .add(PromotionDeleteRequested(state.items[i].id)),
                ),
              ),
            );
          }
          return const SizedBox.shrink();
        },
      ),
    );
  }
}
